<template>
  <div class="mu-pagination" v-if="total">
    ...
    <page-item v-if="totalPageCount > defaultMaxCount && actualCurrent - 1 >= defaultMaxCount-1" identifier="backs"
               @click="handleClick" title="前5页">
      <span>...</span>
    </page-item>
    <page-item v-for="item in pageList" :key="item" :index="item" @click="handleClick"
               :isActive="actualCurrent === item"/>

    <page-item v-if="totalPageCount > defaultMaxCount && totalPageCount - actualCurrent > defaultMaxCount-1"
               identifier="forwards" @click="handleClick" title="后5页">
      <span>...</span>
    </page-item>
