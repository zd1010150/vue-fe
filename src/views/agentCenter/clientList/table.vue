            ...
            <chp-table-body>
              <chp-table-row v-for="(row, rowIndex) in histories" :key="rowIndex">
                <chp-table-cell v-for="(column, columnIndex) in row" :key="columnIndex"
                                :chp-numeric="columnIndex != 'name' " :class="columnIndex">
                  {{column}}
                </chp-table-cell>
              </chp-table-row>
            </chp-table-body>
  ...
  export default {
    ...
    methods: {
      ...
      async fetchData () {
        ...
        let {data, success} = await mt4Service.getAffiliateList(this.innerAgentId, this.model, this.pageSize, this.pageIndex, this.sort)
