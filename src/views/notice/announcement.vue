<template>
  <chp-data-table slot="body" :isDisplayFilterToolbar="isDisplayFilterToolbar" :pageSize="pageSize"
                  :rowsTotal="rowsTotal" :pageOptions="pageOptions" :canFilter="false" :canAdd="false"
      ...
      <chp-table-body>
        <chp-table-row v-for="(row, rowIndex) in noticeList" :key="rowIndex" :mu-select-fieldion="chpSelection">
          <chp-table-cell v-for="(column, columnIndex) in row" :key="columnIndex" :class="columnIndex"
                          :chp-numeric="columnIndex == 'top_up_amount' ">
...
<script>
  ...
  import { SET_NOTICE_REFRESH_FLAG } from 'store/mutation-types'
    ...
    methods: {
      ...
      async fetchAnnoucement () {
        ...
        if (success) {
          ...
          let {success} = await notificationService.markReaded('announcement', this.language)
          if (success) {
