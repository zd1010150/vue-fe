            ...
            <chp-table-row v-for="(row, rowIndex) in innerTrainings"
              ...
              <chp-table-cell v-for="(column, columnIndex) in row" :key="columnIndex">
                ...
                <template v-else-if="columnIndex == 'subject'">
                  ...
                  <i class="fa fa-heart animated infinite zoomIn text-danger pl-xs" aria-hidden="true"
                     v-if="trainings[rowIndex].marked"></i>
                </template>
  ...
  export default {
    ...
    methods: {
      ...
      async register (rowIndex) {
        let id = this.trainings[rowIndex].id, {success} = await trainingService.registerOnlineTraining(id)
        if (success) {
          this.$emit('refresh')
