        ...
        <div class="panel-body">
          <div v-if="value.hasAward" class="tag">
            <div class="tag-box active" @click="getBonus(key)">
              ...
              <div class="word">
									<span class="level">
										{{ $t('levelUp') }} {{ value.awardLevel}} <i class="fa fa-caret-right"
                                                                 aria-hidden="true"></i> {{ value.currentLevel}}
									</span>
                <span>
...
<script>
  ...
  export default {
    ...
    methods: {
      async fetchData () {
        let {data, success} = await activityService.getTraderLevelActivity({
          type: 'all'
        })
        if (success) {
          this.dataInfo = data
      ...
      async getBonus (type) {
        ...
        let {success} = await activityService.traderAward({type: type})
      ...
      async showTerms () {
        let {data, success} = await activityService.getTraderLevelActivityTerms(this.$store.state.language === 'en' ? 'en' : 'zh')
        if (success) {
          this.terms = data
