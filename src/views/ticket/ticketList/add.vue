    ...
    <chp-panel :canCollapse="false" :canClose="false" :isLoading="loadingStatus">
      ...
      <form slot="body" class="form-horizontal form-bordered ">
        ...
        <div class="form-group" :class="errorClass('questionType')">
          ...
          <div class="col-md-6">
            <mu-select-field v-model="model.type" v-validate="'required'" data-vv-value-path="model.type"
                             data-vv-name="questionType">
              <template v-for="(q,key) in questionType">
                <mu-menu-item :value="q.code" :title="q.text" key="key"/>
              </template>
            </mu-select-field>
            <span slot="required" class="error"
                  v-if="errors.has('questionType:required')">{{errors.first('questionType:required')}}</span>
          </div>
        </div>
  ...
  export default {
    ...
    methods: {
      async submit () {
        ...
        if (validateResult) {
          let {success} = await ticketService.addTicket(this.model)
          if (success) {
